...
namespace PlaybookSystem
{
    public partial class wfLayout : Form
    {
        ...
        private void Inicio()
        {
            ...
            if (_dtConf.Rows.Count == 0)
            {
                MessageBox.Show(_gs.MessageText(this.Name, txtWO.Name, "err1"), Text, MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void ShowTimer()
        {
            ...
            try
            {
                ...
                if (!bForm)
                {
                    wfTimer wTimer = new wfTimer();
                    wTimer._dCycle = decimal.Parse(lblCycleTime.Text.ToString());
                    wTimer.Show();
                }
            }
            catch (Exception ex)
        ...
        private void btnTimer_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    //iterate through
                    if (frm.Name == "wfLineHour")
                        bForm = true;
                }

                if(!bForm)
                {
                    wfLineHour Linehr = new wfLineHour();
                    if(!string.IsNullOrEmpty(txtWO.Text.ToString()))
                        Linehr._lsOrden = txtWO.Text.ToString();
        ...
        private void btnTimerWO_Click(object sender, EventArgs e)
        {
            ShowTimer();
        }
        ...
        private void dgwWO_ColumnAdded(object sender, DataGridViewColumnEventArgs e)
        {
            dgwWO.Columns[e.Column.Index].SortMode = DataGridViewColumnSortMode.NotSortable;
        }

        private void dgwItem_ColumnAdded(object sender, DataGridViewColumnEventArgs e)
        {
            dgwItem.Columns[e.Column.Index].SortMode = DataGridViewColumnSortMode.NotSortable;
        }
